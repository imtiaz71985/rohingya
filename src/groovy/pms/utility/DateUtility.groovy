...
class DateUtility {
    ...
    public static final String dd_MM_yyyy_hh_mm_a_SLASH = "dd/MM/yyyy hh:mm a"
    ...
    public static Date parseMaskedDateTime(String paramDate) {

        if (!paramDate) {
            return null
        }

        SimpleDateFormat simpleDF = new SimpleDateFormat(dd_MM_yyyy_hh_mm_a_SLASH)  // "dd/MM/yyyy hh:mm"
    ...
    public static Timestamp getSqlDateWithOneSecond(Date dt) {
        if (!dt) return null
        Calendar calDate = Calendar.getInstance()
        calDate.setTime(dt)
