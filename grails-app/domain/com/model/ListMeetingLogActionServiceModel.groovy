...
class ListMeetingLogActionServiceModel {
    ...
    CREATE OR REPLACE VIEW list_meeting_log_action_service_model AS
          ...
          cat.name meeting_cat,s.id service_id,s.name service,ml.held_on,ml.log_str,ml.desc_str,ml.issues,ml.schedule_id,ml.attendees_others,ml.attendees,
          ms.project_name,ms.team_name,ml.participants,  (SELECT GROUP_CONCAT(NAME SEPARATOR ', ') FROM mis.employee
          ...
          LEFT JOIN meeting_schedule ms ON ml.schedule_id=ms.id
