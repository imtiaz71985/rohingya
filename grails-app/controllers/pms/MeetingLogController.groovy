...
import actions.meetingSchedule.CreateMeetingScheduleActionService
import actions.meetingSchedule.ListMeetingScheduleActionService
import actions.meetingSchedule.UpdateMeetingScheduleActionService
import com.pms.*
...
class MeetingLogController extends BaseController {
    ...
    CreateMeetingScheduleActionService createMeetingScheduleActionService
    UpdateMeetingScheduleActionService updateMeetingScheduleActionService
    ListMeetingScheduleActionService listMeetingScheduleActionService
    ...
    def showSchedule() {
        ...
        lstEmployee = secUserService.currentDepartmentEmpList(user)
        lstEmployee.remove(0)
        render(view: "/meetingSchedule/show", model: [isAdmin:isAdmin,
                                                 lstEmployee: lstEmployee as JSON
        ])
    }
    def createSchedule() {
        renderOutput(createMeetingScheduleActionService, params)
    }
    def updateSchedule() {
        renderOutput(updateMeetingScheduleActionService, params)

    }
    def listSchedule() {
        renderOutput(listMeetingScheduleActionService, params)
    }

    def showScheduledMeeting() {
        def loggedUser = springSecurityService.principal
        SecUser user = SecUser.read(loggedUser.id)
        long serviceId= baseService.serviceIdByRole()
        boolean isAdmin = baseService.isUserSystemAdmin(user.id)
        ...
        lstEmployee.remove(0)
        render(view: "/meetingSchedule/meetingRecord/show", model: [isAdmin:isAdmin,
                                                 serviceId:serviceId,
                                                 lstEmployee: lstEmployee as JSON,
                                                 meetingTypeId:meetingType.id])
    }

    def listByScheduleMeeting() {
        long sId = Long.parseLong(params.scheduleId.toString())
        List<MeetingSchedule> listValue = MeetingSchedule.findAllById(sId)
