...
@Transactional
class GetDropDownMeetingScheduleTaglibActionService extends BaseService implements ActionServiceIntf {
    ...
    private List<GroovyRowResult> listServices() {
        String queryForList = """
            SELECT id,CONCAT(scheduled_date,'(',issues,')') AS name FROM meeting_schedule
             WHERE id NOT IN (SELECT schedule_id FROM meeting_log WHERE is_uploaded<>FALSE)
