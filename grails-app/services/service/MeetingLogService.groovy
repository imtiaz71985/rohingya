...
SELECT COUNT(id) c FROM meeting_log WHERE service_id=${serviceId} AND
 (held_on BETWEEN DATE('${startDate}') AND DATE('${endDate}') OR end_date BETWEEN DATE('${startDate}') AND DATE('${
            endDate
        }'))
        """
...
SELECT COUNT(id) c FROM meeting_log WHERE service_id=${serviceId}  AND id<> ${meetingId} AND
 (held_on BETWEEN DATE('${startDate}') AND DATE('${endDate}') OR end_date BETWEEN DATE('${startDate}') AND DATE('${
    ...
    public List<GroovyRowResult> lstDepartmentWeeklyMeetingStatus(String yearStr, long meetingTypeId) {
        ...
        String query = """"""
        if (meetingType.name.equals("Scheduled Meeting")) {
            query = """
                SELECT l.id,0 VERSION,l.held_on heldOn,l.upload_date uploadDate,l.desc_str descStr,l.issues,l.log_str,l.attendees_others,ms.scheduled_date
                   ,ms.project_name as projectName,ms.team_name as teamName,l.participants FROM meeting_schedule ms
                    LEFT JOIN meeting_log l ON ms.id=l.schedule_id
                    WHERE (YEAR(ms.scheduled_date)=${year} OR YEAR(l.held_on)=${year}) AND l.meeting_type_id = ${
                meetingTypeId
            } AND l.is_uploaded=TRUE
                ORDER BY l.held_on DESC
                """
        } else if (meetingType.name.equals("Quarterly") || meetingType.name.equals("Annually")) {
            query = """
            ...
            query = """
                ...
                FROM (
                    ...
                    LEFT JOIN meeting_log l ON l.service_id = ss.id AND l.is_uploaded=TRUE AND DATE_FORMAT(l.held_on,'%Y') = ${
                year
    ...
    public String getAttendeesOfficialEmail(String attendees) {
        String query = """
                SELECT GROUP_CONCAT(official_email) as email FROM employee WHERE FIND_IN_SET(id, '${attendees}')
                """

        List<GroovyRowResult> lst = groovySql_mis.rows(query)
        String contactEmail = ""
        if (lst.size() > 0)
            contactEmail = lst[0].email
        return contactEmail
    }

    public Void sendMail(String contactEmail, MeetingSchedule meetingSchedule) {

        List<String> recipients = contactEmail.split(',').collect { it.trim() }
        String mailBody = """${meetingSchedule.descStr}"""
        ...
        Thread trd = new Thread() {
            public void run() {
                mailService.sendMail {
                    ...
                    subject "${meetingSchedule.issues}"
                    html(mailBody)
                    attach "meeting_invitation.ics", "text/calendar", emailFile(meetingSchedule)
    ...
    byte[] emailFile(MeetingSchedule meetingSchedule) {

        Calendar calendar = Calendar.getInstance();
        calendar.setTime(meetingSchedule.scheduledDate);
        ...
        archivo.calendar {
            events {
                event(  start       : meetingSchedule.scheduledDate,
                        end         : endTime,
                        summary     : meetingSchedule.issues,
                        utc         : true // optional
                ) {
                    organizer(name: 'Friendship', email: 'info@friendship.ngo')
                    reminder(minutesBefore: 30, description: 'Your meeting starts in 30 minutes!')
                }
